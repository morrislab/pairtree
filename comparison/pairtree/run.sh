...
PAIRTREE_CLUSTREL_DIR=$BASEDIR/scratch/results/${BATCH}.clustrel
...
function create_only_clustrel {
  for ssmfn in $PAIRTREE_INPUTS_DIR/*.ssm; do
    runid=$(basename $ssmfn | cut -d. -f1)
    outdir="$PAIRTREE_CLUSTREL_DIR/$runid"
    resultsfn="$outdir/$runid.results.npz"
...
function create_mutrel_from_clustrel {
  for resultsfn in $PAIRTREE_CLUSTREL_DIR/*/*.results.npz; do
    outdir=$(dirname $resultsfn)
    runid=$(basename $resultsfn | cut -d. -f1)
    basepath="${outdir}/${runid}"
...
function main {
  #run_pairtree #| grep python3 | parallel -j2 --halt 2 --eta
  #create_neutree | parallel -j80 --halt 2 --eta

  #create_only_clustrel #| grep -v '^#' | parallel -j80 --halt 2 --eta
  create_mutrel_from_clustrel | sort --random-sort | parallel -j5 --halt 2 --eta
}
