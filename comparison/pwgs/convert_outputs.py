...
def calc_mutrel(results, base_clusters, supervars):
  ...
  for tidx, weight in zip(tidxs, weights):
    ...
    if mutrel is None:
      mutrel = np.zeros(mutrel.shape)
    mutrel += weight * mutrel
...
def main():
  ...
  mutrel = calc_mutrel(results, base_clusters, supervars)

  np.savez_compressed(args.mutrel_fn, mutrel=mutrel)
